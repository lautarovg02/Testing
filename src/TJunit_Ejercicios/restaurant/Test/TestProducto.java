...
public class TestProducto extends Testing {
    ...
    @TestFactory
    Stream<DynamicTest> dynamicTestFromProducts(){
        List<Producto> testList = new ArrayList<>();
        ...
        testList.addAll(List.of(gaseosaCola, agua, lata_coca_cola, lata_sprite, tortilla, hamburguesa_BIG_0, patatas));


        return testList.stream().map(product -> DynamicTest.dynamicTest("Testing: " + product, () -> {
            producto = product;
            float precioDeCompraPorUnidad = producto.getPrecioUnitarioCompra();
            float precioDeVentaPorUnidad = producto.getPrecioUnitarioVenta();
            float porcentajeDeGanancia = calcularPorcentajeGanancia(precioDeVentaPorUnidad, precioDeCompraPorUnidad);
